...
import { StringUtil, gLog } from "zmg_util"
...
export default class ResourceDownloader {
    ...
    // 下载文件对象集合
    private _downloadUnits: string[];
    // 已下载完成对象集合
    private _completeUnits: string[];
    // 下载失败文件对象集合
    private _failedUnits: string[];
    // 需要删除文件对象集合
    private _deleteUnits: string[];
    // 下载完成的文件数量
    private _downloadCompleteNum: number;
    // 下载失败的文件数量
    private _downloadFailedNum: number;
    // 下载失败的次数
    private _failedCount: number;
    // 并发数量当前值
    private _concurrentCurrent: number;
    ...
    public download(storagePath, localManifest, remoteManifest): void {
        ...
        gLog(StringUtil.Format("【更新】共有{0}个文件需要更新", this._downloadUnits.length.toString()));
        gLog(StringUtil.Format("【更新】共有{0}个文件需要删除", this._deleteUnits.length.toString()));
        ...
        for (var key in this._remoteManifest.assets) {
            if (this._localManifest.assets.hasOwnProperty(key)) {
                ...
                if (HotUpdateConfig.debugRes || this._remoteManifest.assets[key]["md5"] != this._localManifest.assets[key]["md5"]) {
                    gLog("【更新】准备下载更新的资源 ", key);
                    this._addDownloadUnits(key);
                }
            } else {
                gLog("【更新】准备下载本是不存在的资源 ", key);
                this._addDownloadUnits(key);
            }
        }
    }
    ...
    private _downloadAsset(): void {
        ...
        var complete = function (asset) {
            ...
            if (HotUpdateConfig.debugProgress)
                gLog(StringUtil.Format("【更新】进度 {0}/{1}，当前有 {2} 个资源并行下载", this._downloadCompleteNum, this._totalUnits, this._concurrentCurrent));
        ...
        var error = function (error) {
            ...
            gLog(StringUtil.Format("【更新】下载远程路径为 {0} 的文件失败，错误码为 {1}", url, error));
            gLog(StringUtil.Format("【更新】进度 {0}/{1}, 总处理文件数据为 {2}", this._downloadCompleteNum, this._totalUnits, this._downloadCompleteNum + this._downloadFailedNum));
    ...
    private _isUpdateCompleted() {
        ...
        if (this._totalUnits == this._downloadCompleteNum) {                                   // 全下载完成
            ...
            gLog("【更新】下载文件总数量　　：", this._totalUnits);
            gLog("【更新】下载成功的文件数量：", this._downloadCompleteNum);
            gLog("【更新】下载失败的文件数量：", this._downloadFailedNum);

            // 更新失败的次数加 1
            this._failedCount++;

            if (this._failedCount < 3) {
                gLog(StringUtil.Format("【更新】更新重试第 {0} 次", this._failedCount.toString()));
    ...
    private _deleteAssets() {
        for (var i = 0; i < this._deleteUnits.length; i++) {
            ...
            if (jsb.fileUtils.removeFile(filePath)) {
                gLog(StringUtil.Format("【更新】版本多余资源 {0} 删除成功", filePath));
            } else {
                gLog(StringUtil.Format("【更新】版本多余资源 {0} 删除失败", filePath));
            }
            ;
        }
    }
    ...
    private _saveAsset(relativePath, asset) {
        if (cc.sys.isNative) {
            ...
            // 存储文件
            jsb.fileUtils.writeStringToFile(asset, storePath)
        }
    }
