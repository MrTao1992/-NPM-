...
async function rollupBuild (isWatch, entry, output, format, typesDir, sourceDir, unRemoveComments, target) {
    ...
    if (isWatch) {
         ...
         */
            ...
            dtsGen({
                ...
                // fs.readFile(path.resolve(typesDirPath, `index.d.ts`), (e, d) => {
                //     let wd = d.toString().replace("implements cc.Component", ",cc.Component");
                //     fs.writeFile(path.resolve(typesDirPath, `index.d.ts`), wd, () => {
                //         console.log("接口转换结束...");
                //     });
                // })
            })
            ...
            readDir(typesDirPath, declareGlobalRegex, importRegex, sigleDtsFilePath)
    ...
    function readDir (typesDirPath, declareGlobalRegex, importRegex, sigleDtsFilePath) {
        let dtsStr = "";
        fs.readdir(typesDirPath, function (err, files) {
            ...
            for (let i = 0; i < files.length; i++) {
                ...
                let tempDir = path.join(typesDirPath, files[i])
                var stat = fs.statSync(tempDir);
                if (stat.isDirectory()) {
                    console.log("处理文件夹==", tempDir)
                    readDir(tempDir, declareGlobalRegex, importRegex, sigleDtsFilePath)
                    continue
                } else {
                    console.log("处理文件", tempDir)
                    str = fs.readFileSync(tempDir, "utf8");
